...
int main(int argc, char** argv)
{
    ...
    {
        op_decl_const(1, "float", &smoothing_coefficient);
        op_decl_const(NVAR, "float", ff_variable);
        op_decl_const(NDIM, "float", ff_flux_contribution_momentum_x);
        op_decl_const(NDIM, "float", ff_flux_contribution_momentum_y);
        op_decl_const(NDIM, "float", ff_flux_contribution_momentum_z);
        op_decl_const(NDIM, "float", ff_flux_contribution_density_energy);
    ...
    for (int i=0; i<levels; i++) {
        op_par_loop(initialize_variables_kernel, "initialize_variables_kernel", op_nodes[i],
                    op_arg_dat(p_variables[i], -1, OP_ID, NVAR, "float", OP_WRITE));
        op_par_loop(zero_5d_array_kernel, "zero_5d_array_kernel", op_nodes[i],
                    op_arg_dat(p_fluxes[i], -1, OP_ID, NVAR, "float", OP_WRITE));
        if (conf.measure_mem_bound) {
            op_par_loop(zero_5d_array_kernel, "zero_5d_array_kernel", op_nodes[i],
                        op_arg_dat(p_dummy_fluxes[i], -1, OP_ID, NVAR, "float", OP_WRITE));
        }
        ...
        if (!conf.legacy_mode) {
            ...
            op_par_loop(calculate_cell_volumes, "calculate_cell_volumes", op_edges[i], 
                        op_arg_dat(p_node_coords[i],   0, p_edge_to_nodes[i], NDIM, "float", OP_READ), 
                        op_arg_dat(p_node_coords[i],   1, p_edge_to_nodes[i], NDIM, "float", OP_READ), 
                        op_arg_dat(p_edge_weights[i], -1, OP_ID,              NDIM, "float", OP_RW),
    ...
    for (int l=0; l<levels; l++) {
        op_par_loop(dampen_ewt, "dampen_ewt", op_edges[l], 
                    op_arg_dat(p_edge_weights[l], -1, OP_ID, NDIM, "float", OP_INC));
        op_par_loop(dampen_ewt, "dampen_ewt", op_bnd_nodes[l], 
                    op_arg_dat(p_bnd_node_weights[l], -1, OP_ID, NDIM, "float", OP_INC));
    }
    ...
    while(i < conf.num_cycles)
    {
        ...
        op_par_loop(copy_double_kernel, "copy_double_kernel", op_nodes[level],
                    op_arg_dat(p_variables[level],     -1, OP_ID, NVAR, "float", OP_READ),
                    op_arg_dat(p_old_variables[level], -1, OP_ID, NVAR, "float", OP_WRITE));
        ...
        for (rkCycle=0; rkCycle<RK; rkCycle++)
        {
            ...
            op_par_loop(compute_bnd_node_flux_kernel, "compute_bnd_node_flux_kernel", op_bnd_nodes[level],
                        ...
                        op_arg_dat(p_variables[level], 0, p_bnd_node_to_node[level], NVAR, "float", OP_READ),
                        op_arg_dat(p_fluxes[level],    0, p_bnd_node_to_node[level], NVAR, "float", OP_INC));
        ...
        op_par_loop(residual_kernel, "residual_kernel", op_nodes[level],
                    op_arg_dat(p_old_variables[level], -1, OP_ID, NVAR, "float", OP_READ),
                    op_arg_dat(p_variables[level],     -1, OP_ID, NVAR, "float", OP_READ),
                    op_arg_dat(p_residuals[level],     -1, OP_ID, NVAR, "float", OP_WRITE));
        if (level == 0) {
            rms = 0.0;
            op_par_loop(calc_rms_kernel, "calc_rms_kernel", op_nodes[level], 
                        op_arg_dat(p_residuals[level], -1, OP_ID, NVAR, "float", OP_READ), 
        ...
        if (levels <= 1) {
            ...
            if(mg_dir == MG_UP)
            {
                ...
                op_par_loop(up_pre_kernel, "up_pre_kernel", op_nodes[level-1], 
                            op_arg_dat(p_variables[level],  0, p_node_to_mg_node[level-1], NVAR, "float", OP_WRITE),
                            op_arg_dat(p_up_scratch[level], 0, p_node_to_mg_node[level-1], 1, "int", OP_WRITE));

                op_par_loop(up_kernel, "up_kernel", op_nodes[level-1], 
                            op_arg_dat(p_variables[level-1], -1, OP_ID, NVAR, "float", OP_READ), 
                            op_arg_dat(p_variables[level],    0, p_node_to_mg_node[level-1], NVAR, "float", OP_INC),
                ...
                op_par_loop(up_post_kernel, "up_post_kernel", op_nodes[level], 
                            op_arg_dat(p_variables[level],  -1, OP_ID, NVAR, "float", OP_INC), 
            ...
            else
            {
                ...
                if (p_edge_to_mg_nodes[level] != NULL) {
                    ...
                    op_par_loop(down_v2_kernel_pre, "down_v2_kernel_pre", op_nodes[level],
                                op_arg_dat(p_residuals_prolonged[level],      -1, OP_ID, NVAR, "float", OP_WRITE),
                    ...
                    op_par_loop(down_v2_kernel, "down_v2_kernel", op_edges[level],
                                op_arg_dat(p_node_coords[level],   0, p_edge_to_nodes[level],    NDIM , "float", OP_READ),
                                op_arg_dat(p_node_coords[level],   1, p_edge_to_nodes[level],    NDIM , "float", OP_READ),
                                op_arg_dat(p_node_coords[level+1], 0, p_edge_to_mg_nodes[level], NDIM , "float", OP_READ),
                                op_arg_dat(p_node_coords[level+1], 1, p_edge_to_mg_nodes[level], NDIM , "float", OP_READ),
                                op_arg_dat(p_residuals[level+1],   0, p_edge_to_mg_nodes[level], NVAR , "float", OP_READ),
                                op_arg_dat(p_residuals[level+1],   1, p_edge_to_mg_nodes[level], NVAR , "float", OP_READ),
                                op_arg_dat(p_residuals_prolonged[level],      0, p_edge_to_nodes[level], NVAR, "float", OP_INC),
                                op_arg_dat(p_residuals_prolonged[level],      1, p_edge_to_nodes[level], NVAR, "float", OP_INC), 
                    ...
                    op_par_loop(down_v2_kernel_post, "down_v2_kernel_post", op_nodes[level],
                                op_arg_dat(p_residuals_prolonged[level],      -1, OP_ID, NVAR, "float", OP_READ),
                    ...
                    op_par_loop(down_kernel, "down_kernel", op_nodes[level], 
                                op_arg_dat(p_variables[level],    -1, OP_ID, NVAR, "float", OP_INC), 
                                op_arg_dat(p_residuals[level],    -1, OP_ID, NVAR, "float", OP_READ), 
                                op_arg_dat(p_node_coords[level],  -1, OP_ID, NDIM, "float", OP_READ), 
                                op_arg_dat(p_residuals[level+1],   0, p_node_to_mg_node[level], NVAR, "float", OP_READ),
                                op_arg_dat(p_node_coords[level+1], 0, p_node_to_mg_node[level], NDIM, "float", OP_READ));
                }
    ...
    if (conf.validate_result) {
        ...
        for (int l=0; l<levels; l++) {
            ...
            op_par_loop(count_bad_vals, "count_bad_vals", op_nodes[l], 
                        op_arg_dat(p_variables[l], -1, OP_ID, NVAR, "float", OP_READ), 
                        op_arg_gbl(&bad_val_count, 1, "int", OP_INC));
            if (bad_val_count > 0) {
        ...
        if (!value_check_failed) {
            ...
            for (int l=0; l<levels; l++) {
                ...
                op_par_loop(identify_differences, "identify_differences", op_nodes[l], 
                            op_arg_dat(p_variables[l],       -1, OP_ID, NVAR, "float", OP_READ), 
                            op_arg_dat(variables_correct[l], -1, OP_ID, NVAR, "float", OP_READ),
                            op_arg_dat(variables_difference, -1, OP_ID, NVAR, "float", OP_WRITE));

                int count = 0;
                op_par_loop(count_non_zeros, "count_non_zeros", op_nodes[l], 
                            op_arg_dat(variables_difference, -1, OP_ID, NVAR, "float", OP_READ), 
